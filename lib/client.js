...
Client.prototype._defineMethod = function(method, location) {
    var self = this;
    return function(args,headers,callback) {
        if (typeof args === 'function') {
            callback = args;
            args = {};
        }
        self._invoke(method, args, location,headers, function(error, result, raw) {
            callback(error, result, raw);
        })
    }
}

Client.prototype._invoke = function(method, arguments, location,exheaders, callback) {
    var self = this,
        name = method.$name,
        input = method.input,
    ...
    if (input.parts) {
      ...
      // Modifying this part to use it with string  
      //message = arguments;
      message = '<'+input.targetNSAlias+':'+input.$name+'>';
      message+=arguments;
      message+='</'+input.targetNSAlias+':'+input.$name+'>';
    ...
    http.request(location, xml, function(err, response, body) {
        if (err) {
            ...
            try {
                ...
                // This is not correct as it doesnot parse XML to JSON correctly
                //For Example : <item System="" Id="">

                var obj ;

                // Used a Parsing library 
                var xml = body;
